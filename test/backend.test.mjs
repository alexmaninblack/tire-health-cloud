...
test("unknown schema and unknown table deny readiness and foundation-only removal proof", async () => {
  for (const mutation of ["PRAGMA user_version=5", "CREATE TABLE product_data(value TEXT)"]) {
    ...
    try {
      ...
      else assert.equal(observed.prepare("PRAGMA user_version").get().user_version, 5);
      observed.close();
    } finally {if (app) await app.shutdown(); rmSync(directory, {recursive: true, force: true});}
  }
});
